...
class BacktestEngine:
    ...
    def portfolio_management(self, _test_data, random_pick=False, rank_by='Expectancy'):
        ...
        for _, df in _test_data.groupby('publish_date_and_time'):
            # Debug
            # df = test_data[test_data['publish_date_and_time'] == datetime(2021, 4, 12)]
            #
            if random_pick:
                df['Expectancy'] = 0
            positive_expectancy = df[['exch_region', 'side', rank_by]].copy(deep=True)
