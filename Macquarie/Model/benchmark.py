...
def benchmark_expectancy(train_data=None, test_data=None):
    ...
    #
    # elif train_data is None:  # Testing data is defined, use all data
    ...
    case_unfound = set(combinations).difference(set(analyst_long_expectancy.index))
    case_unfound_df = pd.DataFrame(0, index=case_unfound, columns=analyst_long_expectancy.columns)

    analyst_long_expectancy = pd.concat([analyst_long_expectancy, case_unfound_df], axis=0)
    analyst_short_expectancy = pd.concat([analyst_short_expectancy, case_unfound_df], axis=0)

    test_data['top_analyst_long'] = [
        analyst_long_expectancy.loc[x['Head analyst']]['Expectancy'] for _, x in test_data[['Head analyst']].iterrows()]
    test_data['top_analyst_short'] = [
        analyst_short_expectancy.loc[x['Head analyst']]['Expectancy'] for _, x in test_data[['Head analyst']].iterrows()]
    ###########################################################################
    ...
    for k in range_of_test:
        ...
        if not from_local:
            ...
            testing_data_long['Expectancy'] = [
                blind_long_expectancy.loc[(x['Headline sentiment'], x['Summary sentiment'], x['Report Type'])][
                    'Expectancy']
                for _, x in testing_data_long[['Headline sentiment', 'Summary sentiment', 'Report Type']].iterrows()]

            testing_data_short['Expectancy'] = [
                blind_short_expectancy.loc[(x['Headline sentiment'], x['Summary sentiment'], x['Report Type'])][
                    'Expectancy']
                for _, x in testing_data_short[['Headline sentiment', 'Summary sentiment', 'Report Type']].iterrows()]
            ...
            # us_mask = (output['exch_region'] == 'Americas')
            ...
            final_trades = Engine.portfolio_management(output, random_pick=False, rank_by='Expectancy')
...
if __name__ == '__main__':
    ...
    # simulation_result = simulation(range(201, 501), from_local=False)
    ...
    # fig.savefig(f'{DL.database_path}/Backtest/Simulation/Simulation total (blind).png')
    #
