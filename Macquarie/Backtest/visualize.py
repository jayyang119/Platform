...
class visual:
    ...
    def plot_hit_ratio(self, column='d0_r'):
        ...
        long_hit_ratio, short_hit_ratio = benchmark_side(self.trades_df)
    ...
    def plot_side_expectancy(self, column='d0_r'):
        ...
        expectancy_by_side = get_expectancy(self.trades_df, group_by=[])
        ...
        expectancy_by_side = get_expectancy(self.trades_df, group_by=['side'])
        print(expectancy_by_side)
    ...
    def plot_market_expectancy(self, column='d0_r'):
        ...
        expectancy_by_market = get_expectancy(self.trades_df, group_by=['exch_location'])
        ...
        expectancy_by_market_side = get_expectancy(self.trades_df, group_by=['exch_location', 'side'])
...
def get_matrix_metric(_df, by_i):
    ...
    total = get_expectancy(_df, group_by=['exch_location', 'Headline sentiment', 'Summary sentiment'])[['Hit ratio', 'Expectancy', 'Count']].unstack()

    total_head = get_expectancy(_df, group_by=['exch_location', 'Headline sentiment'])[['Hit ratio', 'Expectancy', 'Count']].unstack()

    total_sum = get_expectancy(_df, group_by=['exch_location', 'Summary sentiment'])[['Hit ratio', 'Expectancy', 'Count']].unstack()
    ...
    for side in trade_df['side'].unique():
        ...
        for by_i in df[by].unique():
            ...
            for _count in range(len(rt_list)):
                ...
                df_rt = df_i[df_i['Report Type'] == report_type]
                if len(df_rt) == 0:
                    continue
