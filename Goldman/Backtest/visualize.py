...
class visual:
    ...
    def plot_r_dist(self, column='d0_r'):
        ...
        axs.title.set_text(f'R distribution {self.strategy} - Hold 1 day')
    ...
    def plot_hit_ratio(self, column='d0_r'):
        total_hit_ratio = hit_ratio(self.trades_df, column)
        ...
        fig = plot_EDA([long_hit_ratio, total_hit_ratio, short_hit_ratio],
                        ...
                        title=f'Hit ratio plot (Hold 1 day) - {self.strategy}', precision=4)
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/hit ratio.png')

    def plot_side_expectancy(self, column='d0_r'):
        expectancy_by_side = get_expectancy(self.trades_df, column, inputs=[column], group_by=[])
        fig = plot_EDA(expectancy_by_side['Expectancy'], labels=['Total'],
                       precision=4, title=f'Total Expectancy - {self.strategy}')
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/Total expectancy.png')

        expectancy_by_side = get_expectancy(self.trades_df, column, inputs=[column, 'side'], group_by=['side'])
        fig = plot_EDA(expectancy_by_side.reset_index()['Expectancy'], labels=['Long', 'Short'],
                       precision=4, title=f'Expectancy by sides - {self.strategy}')
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/expectancy by side.png')
    ...
    def plot_market_r(self, column='d0_r'):
        ...
        fig = plot_EDA_country(R_by_market_side.reset_index(), hue='side',
                               title=f'No. of trades by Country/side - {self.strategy}', rotate=True)
        ...
        fig = plot_EDA_country(R_by_market_side.reset_index(), y=column, hue='side',
                               title=f'R by Country/side - {self.strategy}', rotate=True)
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/R by market and side.png')

    def plot_market_expectancy(self, column='d0_r'):
        expectancy_by_market = get_expectancy(self.trades_df, column, inputs=['No. of trades', column, 'exch_location'],
                                              group_by=['exch_location'])
        fig = plot_EDA_country(expectancy_by_market.reset_index(), x='exch_location', y='Expectancy',
                               title=f'Expectancy by market - {self.strategy}', precision=2, rotate=True)
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/expectancy by market.png')

        expectancy_by_market_side = get_expectancy(self.trades_df, column,
                                                   inputs=['No. of trades', column, 'exch_location', 'side'],
                                                   group_by=['exch_location', 'side'])
        fig = plot_EDA_country(expectancy_by_market_side.reset_index(), x='exch_location', y='Expectancy',
                               hue='side', title=f'Expectancy by market - {self.strategy}', precision=2, rotate=True)
    ...
    def plot_region_expectancy(self, column='d0_r'):
        expectancy_by_region = get_expectancy(self.trades_df, column, inputs=['No. of trades', column, 'exch_region'],
                                              group_by=['exch_region'])
        fig = plot_EDA_country(expectancy_by_region.reset_index(), x='exch_region', y='Expectancy',
                               title=f'Expectancy by region - {self.strategy}', precision=2, rotate=True)
        fig.savefig(f'{BACKTEST_PATH}/{self.strategy}/expectancy by region.png')

        expectancy_by_region_side = get_expectancy(self.trades_df, column,
                                                   inputs=['No. of trades', column, 'exch_region', 'side'],
                                                   group_by=['exch_region', 'side'])
        fig = plot_EDA_country(expectancy_by_region_side.reset_index(), x='exch_region', y='Expectancy',
                               hue='side', title=f'Expectancy by region - {self.strategy}', precision=2, rotate=True)
...
def get_matrix_metric(_df, by_i, by):
    count = len(_df)
    hit = hit_ratio(_df)
    expectancy = get_expectancy(_df, 'd0_r')['Expectancy']
    total_dict = {0: hit, 1: expectancy, 2: count}

    total = get_expectancy(_df, 'd0_r',
                                   inputs=['No. of trades', 'd0_r', 'Report Type', by,
                                           'Headline sentiment', 'Summary sentiment'],
                                   group_by=[by, 'Headline sentiment', 'Summary sentiment'],
                                   )[['Hit ratio', 'Expectancy', 'Count']].unstack()

    total_head = get_expectancy(_df, 'd0_r',
                                        inputs=['No. of trades', 'd0_r', 'Report Type', by,
                                                'Headline sentiment',
                                                'Summary sentiment'],
                                        group_by=[by, 'Headline sentiment'],
                                        )[['Hit ratio', 'Expectancy', 'Count']].unstack()

    total_sum = get_expectancy(_df, 'd0_r',
                                       inputs=['No. of trades', 'd0_r', 'Report Type', by,
                                               'Headline sentiment',
                                               'Summary sentiment'],
                                       group_by=[by, 'Summary sentiment'],
                                       )[['Hit ratio', 'Expectancy', 'Count']].unstack()
...
def plot_matrix(strategy, by='exch_location'):
    ...
    trade_df = trade_df[trade_df['exch_region'].isin(['Americas', 'Europe', 'Asia'])].reset_index(drop=True)
    ...
    for side in trade_df['side'].unique():
        ...
        rt_list = ['Earning\'s Review', 'Rating Change', 'Target Price Increase', 'Target Price Decrease', 'Estimate Change', 'ad-hoc']
        for by_i in df[by].unique():
            ...
            if len(df_i) == 0:
                continue
            (k_hit, k_expectancy, k_count) = get_matrix_metric(df_i, by_i, by)
            logger.info(k_hit)
            logger.info(k_expectancy)
            logger.info(k_count)

            if side == 'long':
                k_count.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=5)
            k_expectancy.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=13)
            k_hit.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=21)


            for _count in range(len(rt_list)):
                report_type = rt_list[_count]
                df_rt = df_i[df_i['Report Type'] == report_type].copy(deep=True)
                if len(df_rt) == 0:
                    continue
                (k_hit, k_expectancy, k_count) = get_matrix_metric(df_rt, by_i, by)
                # rt_name = ''.join([x[0].upper() for x in report_type.split(' ')])
                if side == 'long':
                    k_count.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=5+(_count+1)*24)
                k_expectancy.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=13+(_count+1)*24)
                k_hit.to_excel(writer, sheet_name=f'{by_i}', startcol=3, startrow=21+(_count+1)*24)
...
if __name__ == '__main__':
    ...
    #     output = benchmark_expectancy(df, column, inputs=['No. of trades', column, 'exch_region'], group_by=['exch_region'])
    #     print(group, output)
    #
