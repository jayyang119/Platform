...
class GSPriceDf:
    ...
    def add_new_columns(self, _df):
        ...
        DS = Dataset(df)
        DS.clean(mode='eikon', tz_name='America/New_York')
    ...
    def GS_update_price_df(self, update=False):
        ...
        if not DL.checkDB('price_df.csv'):
            ...
            price_df_new = pd.concat([self._sentiment_df, price_df[self._sentiment_df.columns]], axis=0).drop_duplicates(
                ['publish_date_and_time', 'headline'], keep=False)
        ...
        self._price_df = self._price_df.sort_values('publish_date_and_time', ascending=False)
    ...
    def GS_predict_price_df(self, days=3):
        ...
        results.sort_values('publish_date_and_time', ascending=False, inplace=True)
