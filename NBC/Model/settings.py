...
class DataCleaner:
    ...
    def get_benchmark_test_data(self, update=False):
        ...
        if not DL.checkDB('Backtest/Benchmark data/X_train.csv') or update:
            df = DL.loadDB('price_df.csv', parse_dates=['publish_date_and_time'])
            df = self.preprocess_trade_df(df)
            df = df.sort_values('publish_date_and_time', ascending=True)

            DL.create_folder(f'{DL.database_path}/Backtest/Benchmark data')
            # Blind long
            logger.info('Calculating blind long pnl..')
            df['side'] = 'long'
            df = get_pnl(df)
            DL.toBT(df, 'Benchmark data/blind long')

            # Blind short
            logger.info('Calculating blind short pnl..')
            df['side'] = 'short'
            df = get_pnl(df)
            ...
            logger.info('Calculating benchmark pnl..')
            ...
            train_data = DL.loadDB('Backtest/Benchmark data/X_train.csv', parse_dates=['publish_date_and_time'])
            test_data = DL.loadDB('Backtest/Benchmark data/X_test.csv', parse_dates=['publish_date_and_time'])
    ...
    def get_model_test_data(self, strategy):
        if DL.checkDB(f'Backtest/{strategy}.csv'):
            test_data = DL.loadDB(f'Backtest/{strategy}.csv', parse_dates=['publish_date_and_time'])
        else:
            test_data = DL.loadDB('Backtest/Model data/X_test.csv', parse_dates=['publish_date_and_time'])
