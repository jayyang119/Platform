...
class DataLoader(Logger):
    ...
    def loadBT(self, file):
        ...
        if self.checkDB(f'Backtest/{file}.csv'):
            df = pd.read_csv(_path, parse_dates=['Date', 'Time'])
            return df
        else:
            self.info(f"{_path} directory error. Please check.")
            return []
    ...
    def loadTickers(self):
        ...
        # valid_tickers = all_tickers[~all_tickers['Industry'].isin([''])]
        # valid_tickers = all_tickers[~all_tickers['Delisted'].isin(['Y'])]

        # valid_tickers = valid_tickers.drop_duplicates()
        # delisted = all_tickers[all_tickers['Delisted'].isin(['Y'])]

        return all_tickers

    def loadExistingTickers(self):
        existing_tickers = glob.glob(f'{self.daily_path}/*.csv')
        existing_tickers = [os.path.basename(x).rstrip('.csv') for x in existing_tickers]
