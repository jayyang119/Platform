...
def benchmark_side(df, column='d0_r'):
    ...
    print(f"Long trades hit ratio {column} %.4f" % long_hit_ratio)

    short_hit_ratio = hit_ratio(short_trades_df, column)
    print(f"Short trades hit ratio {column} %.4f" % short_hit_ratio)

    return long_hit_ratio, short_hit_ratio


def get_expectancy(_df, group_by=[], column='d0_r'):
